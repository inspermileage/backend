...
def test_update_invalid_one(db: Session) -> RoundModel:
    item_id=123456789
    item_update= RoundUpdate(name= random_lower_string(), description= random_lower_string())
    with pytest.raises(NonExistenceException):
        updated_round = update(db_session=db, round_id=item_id, obj_in=item_update)
