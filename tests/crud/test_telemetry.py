...
def test_create_item(db_session: Session) -> TelemetryModel:
    ...
    battery = 60
    round_id = round.id
    item_in = TelemetryCreate(speed=speed, distance=distance, engine_temp=engine_temp,
                              creation_time=creation_time, energy_cons=energy_cons, rpm=rpm, battery=battery, round_id=round_id)
...
def test_read_one_by_id(db_session: Session) -> TelemetryModel:
    ...
    assert read_item.creation_time.strftime(
        "%Y-%m-%d %H:%M:%S") == creation_time
